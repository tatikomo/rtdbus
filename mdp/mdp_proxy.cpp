...
int main (int argc, char *argv [])
{
    ...
    try
    {
      ...
      /*
       * Начало работы
       * 1. Попытка чтения снимка БД, или его создание (включая НСИ);
       * 2. Связывание с сокетом;
       *    Строка подключения может храниться в снимке БД Брокера,
       *    читаемом при старте, или браться значение по-умолчанию
       *    "tcp://lo:5555" при инициализации БД.
       *
       * NB: Точка подключения по-умолчанию использует локальный интерфейс,
